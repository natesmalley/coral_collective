...
class AgentRunner:
    def __init__(self):
        ...
        self.session_data = {"start_time": datetime.now(), "interactions": []}
    ...
    def load_agents_config(self) -> Dict:
        ...
        if not config_path.exists():
            console.print(
                "[yellow]Configuration file not found. Initializing CoralCollective...[/yellow]"
            )
            if self.initialize_config():
                ...
                console.print(
                    "[red]Failed to initialize configuration. Creating default config...[/red]"
                )
        ...
        try:
            ...
            console.print(
                "[yellow]Run 'python agent_runner.py init' to initialize[/yellow]"
            )
            sys.exit(1)
...
{', '.join(agent.get('next_agents', ['Any']))}
        ...
        console.print(
            Panel(Markdown(details), title=f"Agent: {agent_id}", border_style="blue")
        )
    ...
    def get_agent_prompt(self, agent_id: str) -> str:
        ...
        if "prompt_path" in agent:
            ...
            if agent_id in prompt_files:
                ...
                prompt_file = (
                    self.base_path
                    / f'agents/specialists/{agent_id.replace("-", "_")}.md'
                )
    ...
    async def get_agent_bridge(
        self, agent_id: str, mcp_enabled: bool = True
    ) -> Optional[AgentMCPBridge]:
        ...
        try:
            ...
            console.print(
                f"[yellow]Failed to create MCP bridge for {agent_id}: {e}[/yellow]"
            )
            return None
    ...
    async def close_agent_bridge(self, agent_id: str):
        ...
        if agent_id in self.active_bridges:
            ...
            try:
                ...
                console.print(
                    f"[yellow]Error closing MCP bridge for {agent_id}: {e}[/yellow]"
                )
            finally:
            ...
            try:
                ...
                if agent_bridge:
                    ...
                    if mcp_tools_section:
                        console.print(
                            f"[green]✓ MCP tools available for {agent_id}[/green]"
                        )
                    else:
                        console.print(
                            f"[yellow]No MCP tools available for {agent_id}[/yellow]"
                        )
            except Exception as e:
...
"""
        ...
        if provider:
            try:
                ...
                if provider == "claude":
                    ...
                    console.print(
                        f"[yellow]Unknown provider '{provider}' - using default stdout rendering[/yellow]"
                    )
                ...
                ctx_section = next(
                    (s for s in sections if s["key"] == "project_context"), None
                )
                header_sections = [
                    s for s in sections if s.get("key") != "project_context"
                ]
                header_text = renderer.render_sections(header_sections)
                header_tokens = estimator.estimate(header_text)
                ctx_tokens = (
                    estimator.estimate(ctx_section["text"]) if ctx_section else 0
                )

                # Decide mode: default streaming; if context + header exceeds window, batch by context parts
                context_exceeds = (
                    (header_tokens + ctx_tokens) > max_input_tokens
                    if ctx_section
                    else False
                )
                ...
                if context_exceeds:
                    ...
                    context_chunks = chunk_text(
                        ctx_section["text"], estimator, chunk_tokens=available_for_ctx
                    )
                    ...
                    for idx, ctx_chunk in enumerate(context_chunks or [""], start=1):
                        ...
                        task_sec = next(
                            (s for s in header_sections if s["key"] == "task"), None
                        )
                        if task_sec:
                        ...
                        renderer.deliver(
                            part_text,
                            mode=mode,
                            base_dir=self.base_path / "prompts",
                            filename_stub=stub,
                        )
                else:
        ...
        if non_interactive:
            ...
            console.print(
                f"[cyan]Agent: {agent['name']} running in automation mode[/cyan]"
            )
            ...
            self.state_manager.record_agent_completion(
                ...
                success=True,
                outputs=result,
                artifacts=[{"type": "prompt", "path": str(prompt_file)}],
            )
        ...
        if (
            os.environ.get("CI")
            or os.environ.get("CORAL_NON_INTERACTIVE")
            or os.environ.get("CORAL_AUTOMATION")
        ):
            console.print(
        ...
        if Confirm.ask("Show full prompt?", default=False):
            console.print(
                Panel(full_prompt[:1000] + "...", title="Agent Prompt Preview")
            )
        ...
        self.collector.record_interaction(
            agent_id, success, satisfaction, task_type, completion_time
        )

        # Collect specific feedback if needed
        if satisfaction <= 6 or not success:
            collect_feedback = Confirm.ask(
                "Would you like to provide specific feedback?", default=True
            )
            if collect_feedback:
                ...
                self.collector.add_feedback(
                    agent_id, issue, suggestion, priority, "user"
                )
        ...
        if Confirm.ask("Did the agent provide handoff instructions?", default=False):
            ...
            handoff = {"next_agent": next_agent, "next_task": next_task}
        ...
        for source_path in possible_paths:
            if source_path.exists():
                ...
                try:
                    ...
                    console.print(
                        f"[green]Copied configuration from {source_path}[/green]"
                    )
                    return True
                except Exception as e:
                    console.print(
                        f"[yellow]Failed to copy from {source_path}: {e}[/yellow]"
                    )
    ...
    def run_workflow(
        self, sequence: List[str], project: Dict, non_interactive: bool = False
    ):
        ...
        context = {"project": project, "completed_agents": [], "outputs": {}}
        ...
        for i, agent_id in enumerate(sequence, 1):
            ...
            result = self.run_agent(
                agent_id, task, context, non_interactive=non_interactive
            )
            ...
            if not result.get("success", True):
                if non_interactive:
                    console.print(
                        f"[yellow]Warning: {agent['name']} reported failure, continuing workflow in non-interactive mode[/yellow]"
                    )
    ...
    def show_session_summary(self):
        ...
        for interaction in interactions:
            ...
            task_preview = (
                interaction["task"][:40] + "..."
                if len(interaction["task"]) > 40
                else interaction["task"]
            )
        ...
        # Summary stats
        avg_satisfaction = sum(i["satisfaction"] for i in interactions) / len(
            interactions
        )
        success_rate = (
    ...
    def dashboard(self):
        ...
        if metrics_path.exists():
            ...
            if "performance_summary" in metrics:
                ...
                console.print(
                    f"• Success rate: {summary.get('overall_success_rate', 'N/A')}"
                )
                console.print(
                    f"• Average satisfaction: {summary.get('average_satisfaction', 'N/A')}"
                )
...
async def async_main():
    ...
    parser.add_argument(
        "--model",
        help="Tokenizer/model hint for token estimation (e.g., openai:gpt-4o, anthropic:claude-3.5-sonnet)",
    )
    parser.add_argument(
        "--validate-tokens",
        action="store_true",
    ...
    if args.command == "init":
        ...
        if config_path.exists():
            console.print(
                f"[green]✓ Configuration already exists at {config_path}[/green]"
            )
        else:
        ...
        if (
            base_path.name == ".coral"
            or (base_path / "standalone_config.json").exists()
        ):
    ...
    if args.command == "interactive":
        ...
        if not args.agent or not args.task:
            console.print(
                "[red]Error: --agent and --task required for run command[/red]"
            )
            sys.exit(1)
        try:
            ...
            if (
                result.get("mcp_metrics")
                and result["mcp_metrics"].get("usage_metrics", {}).get("total_calls", 0)
                > 0
            ):
                ...
                console.print(
                    f"• Servers: {', '.join(metrics.get('servers_used', []))}"
                )
                ...
                try:
                    ...
                    for server_name in servers:
                        ...
                        if server_info:
                            status = (
                                "🟢 Connected"
                                if server_info.get("connection_stats", {}).get(
                                    "connected"
                                )
                                else "🔴 Disconnected"
                            )
                    ...
                    for agent in sample_agents:
                        ...
                        if tools:
                            console.print(
                                f"  • {agent}: {len(tools)} servers accessible"
                            )
