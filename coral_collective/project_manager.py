...
class ProjectManager:
    ...
    def save_project(self, project: Dict):
        """Save a project"""
        project_file = (
            self.projects_dir / f"{project['name'].replace(' ', '_').lower()}.yaml"
        )
    ...
    def create_project(self) -> Dict:
        ...
        type_choice = Prompt.ask(
            "Select project type", choices=list(project_types.keys())
        )
        ...
        project = {
            ...
            "agents_used": [],
            "tasks_completed": [],
            "metrics": {"total_time": 0, "success_rate": 0, "satisfaction": []},
            "notes": [],
    ...
    def list_projects(self, status_filter: Optional[str] = None):
        ...
        for name, project in self.active_projects.items():
            ...
            created_date = datetime.fromisoformat(project["created"]).strftime(
                "%Y-%m-%d"
            )
    ...
    def show_project_details(self, project_name: str):
        ...
        for phase_num, phase_data in project["phases"].items():
            phase_node = phases.add(
                f"Phase {phase_num}: {phase_data['name']} [{phase_data['status']}]"
            )
        ...
        if project["agents_used"]:
            ...
            for agent_entry in project["agents_used"]:
                agents.add(
                    f"• {agent_entry['agent']} - {agent_entry.get('task', 'N/A')[:50]}..."
                )
        ...
        if project["metrics"]["satisfaction"]:
            avg_satisfaction = sum(project["metrics"]["satisfaction"]) / len(
        ...
        console.print(
            Panel(tree, title=f"Project: {project_name}", border_style="blue")
        )
    ...
    def update_project_status(self, project_name: str, status: str):
        ...
        if status not in valid_statuses:
            console.print(
                f"[red]Invalid status. Choose from: {', '.join(valid_statuses)}[/red]"
            )
            return
    ...
    def add_agent_interaction(
        ...
        project["metrics"]["success_rate"] = int(
            (successful / total_interactions) * 100
        )
    ...
    def add_note(self, project_name: str, note_text: str):
        ...
        note = {"date": datetime.now().isoformat(), "text": note_text}
    ...
    def generate_project_report(self, project_name: str) -> str:
        ...
        report.append(
            f"- **Current Phase**: {project['current_phase']} - {project['phases'][project['current_phase']]['name']}"
        )
        ...
        for phase_num, phase_data in project["phases"].items():
            status_emoji = (
                ...
                else "⏳" if phase_data["status"] == "active" else "⏸️"
            )
            report.append(
        ...
        for entry in project["agents_used"][-10:]:  # Last 10 interactions
            ...
            report.append(
                f"- {success_mark} **{entry['agent']}**: {entry['task'][:60]}... (Satisfaction: {entry['satisfaction']}/10)"
            )
        ...
        if current_phase < 4 and project["status"] != "completed":
            ...
            report.append(
                f"Prepare for Phase {current_phase + 1}: {next_phase['name']}"
            )
    ...
    while True:
        ...
        if choice == "1":
            ...
            format_choice = Prompt.ask(
                "Export format", choices=["yaml", "json", "markdown"]
            )
