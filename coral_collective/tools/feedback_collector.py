...
class FeedbackCollector:
    def __init__(
        self,
        feedback_path: str = "feedback/agent_feedback.yaml",
    ...
    def record_interaction(
        ...
        metrics["avg_satisfaction"] = round(
            total_satisfaction / metrics["total_uses"], 1
        )
        ...
        metrics["tasks_completed"][task_key] = (
            metrics["tasks_completed"].get(task_key, 0) + 1
        )
    ...
    def generate_report(self, agent: Optional[str] = None) -> str:
        ...
        if agent:
            ...
            if agent in metrics_data.get("agent_metrics", {}):
                ...
                report.append(
                    f"- Avg Completion Time: {m.get('avg_completion_time', 'N/A')}"
                )
            ...
            if "performance_summary" in metrics_data:
                ...
                report.append(
                    f"- Overall Success Rate: {s.get('overall_success_rate', 'N/A')}"
                )
                report.append(
                    f"- Average Satisfaction: {s.get('average_satisfaction', 'N/A')}"
                )
...
def main():
    ...
    parser.add_argument(
        "action",
        choices=["feedback", "interaction", "report"],
    ...
    if args.action == "feedback":
        ...
        collector.add_feedback(args.agent, args.issue, args.suggestion, args.priority)
        ...
        collector.record_interaction(
            args.agent, args.success, args.satisfaction, args.task_type, args.time
        )
