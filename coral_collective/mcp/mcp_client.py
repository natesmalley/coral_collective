        ...
        try:
            ...
            if not line:
                raise ConnectionError(
                    f"Connection to {self.server_name} closed unexpectedly"
                )
...
class MCPConnection:
    ...
    async def connect(self) -> bool:
        ...
        async with self.connection_lock:
            ...
            for attempt in range(self.server.retry_attempts):
                try:
                    logger.info(
                        f"Connecting to {self.server.name} (attempt {attempt + 1})"
                    )
                    ...
                    logger.error(
                        f"Connection attempt {attempt + 1} failed for {self.server.name}: {e}"
                    )
                    ...
                    if attempt < self.server.retry_attempts - 1:
                        await asyncio.sleep(
                            self.server.retry_delay * (2**attempt)
                        )  # Exponential backoff
    ...
    async def _initialize_connection(self):
        ...
        init_request = MCPMessage(
            ...
            params={
                ...
                "clientInfo": {"name": "CoralCollective", "version": "1.0.0"},
            },
        )
        ...
        logger.debug(
            f"Server {self.server.name} initialized with capabilities: {response.result}"
        )
        ...
        while self.connected:
            try:
                ...
                ping_request = MCPMessage(
                    id=str(uuid.uuid4()), method="ping", params={}
                )

                try:
                    await asyncio.wait_for(
                        self._send_request(ping_request), timeout=10.0
                    )
    ...
    async def _send_request(
        self, request: MCPMessage, initialize: bool = False
    ) -> MCPMessage:
        ...
        try:
            ...
            response = await asyncio.wait_for(
                response_future, timeout=self.server.timeout
            )
    ...
    async def list_tools(self, use_cache: bool = True) -> List[Dict[str, Any]]:
        ...
        async with self.request_semaphore:
            request = MCPMessage(id=str(uuid.uuid4()), method="tools/list", params={})

            response = await self._send_request(request)

            if response.error:
                logger.error(
                    f"Error listing tools from {self.server.name}: {response.error}"
                )
                audit_logger.warning(
                    f"Tools list error for {self.server.name}: {response.error}"
                )
                return []
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Any:
        ...
        async with self.request_semaphore:
            ...
            if response.error:
                ...
                audit_logger.error(
                    f"Tool call failed: {self.server.name}/{tool_name} - {response.error}"
                )
        ...
        while self.connected and self.transport:
            try:
                ...
                if message.id and message.id in self.pending_requests:
                    ...
                    logger.debug(
                        f"Notification from {self.server.name}: {message.method}"
                    )
                else:
                    logger.warning(
                        f"Unhandled message from {self.server.name}: {message.to_dict()}"
                    )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "uptime_seconds": (
                datetime.now(timezone.utc) - self.transport.stats.connect_time
            ).total_seconds(),
...
class MCPClient:
    ...
    def load_configuration(self):
        ...
        try:
            ...
            for server_name, server_config in self.config.get(
                "mcp_servers", {}
            ).items():
                if server_config.get("enabled", True):
                    self.servers[server_name] = MCPServer(
                        ...
                        max_concurrent_requests=server_config.get(
    ...
    async def send_request(
        ...
        request = MCPMessage(id=str(uuid.uuid4()), method=method, params=params)
...
class AgentMCPInterface:
    ...
    def __init__(self, agent_type: str, client: Optional[MCPClient] = None):
        ...
        audit_logger.info(
            f"Agent interface created: {agent_type} (session: {self.session_id})"
        )
    ...
    def _check_permission(self, server_name: str) -> bool:
        ...
        if not allowed:
            audit_logger.warning(
                f"Permission denied: {self.agent_type} tried to access {server_name}"
            )
        return allowed
    ...
    async def github_create_issue(
        ...
        params = {"title": title, "body": body, "labels": labels or []}
    ...
    async def database_query(self, query: str, params: List[Any] = None) -> List[Dict]:
        ...
        query_params = {"query": query, "params": params or []}

        try:
            result = await self.client.call_tool(
                "postgres", "execute_query", query_params
            )
    ...
    async def docker_run(
        self, image: str, command: str = None, env: Dict[str, str] = None
    ) -> Optional[str]:
        """Run a Docker container"""
        if not self._check_permission("docker"):
            return None

        params = {"image": image, "command": command, "environment": env or {}}
    ...
    async def e2b_execute(self, code: str, language: str = "python") -> Optional[Dict]:
        ...
        params = {"language": language, "code": code}
    ...
    async def close(self):
        ...
        audit_logger.info(
            f"Agent interface closed: {self.agent_type} (session: {self.session_id})"
        )
...
async def main():
    ...
    async with MCPClient() as client:
        ...
        if "brave-search" in servers:
            ...
            try:
                results = await frontend_agent.search_web(
                    "React hooks tutorial", max_results=3
                )
        ...
        for server_name in servers:
            ...
            if info:
                print(f"  {server_name}:")
                print(
                    f"    Connected: {info['connection_stats'].get('connected', False)}"
                )
