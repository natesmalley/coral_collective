...
For more information, see: https://github.com/coral-collective/coral-collective
    ...
    workflow_parser.add_argument('workflow_type', help='Workflow type (full_stack, backend_only, etc.)')
    ...
    # Init command
    init_parser = subparsers.add_parser('init', help='Initialize CoralCollective in current directory')
...
def init_command():
    ...
    if drop_script.exists():
        # Run the drop script
        result = subprocess.run(['bash', str(drop_script)], capture_output=True, text=True)
